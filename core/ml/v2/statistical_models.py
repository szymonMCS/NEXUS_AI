...
from core.ml.v2.models_base import BasePredictor, PredictionResult
...
class StatisticalEnsemble(BasePredictor):
    ...
    def predict(self, features: Dict[str, Any]) -> PredictionResult:
        ...
        if not home_name or not away_name:
            logger.warning("Missing player names in features, returning neutral prediction")
            return PredictionResult(0.5, 0.5, confidence=0.0, model_name=self.name)
        ...
        try:
            ...
            logger.debug(f"StatisticalEnsemble prediction: {home_name} vs {away_name}: "
            ...
            logger.error(f"StatisticalEnsemble prediction error: {e}")
            return PredictionResult(0.5, 0.5, confidence=0.0, model_name=self.name)
